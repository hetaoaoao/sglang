...
class DeepseekMLARocmForwardMixin:
    ...
    def forward_absorb_fused_mla_rope_prepare(
        ...
        _forward_metadata = forward_batch.attn_backend.forward_metadata
        attn_logits = None
        kv_indptr = _forward_metadata.kv_indptr
        kv_indices = _forward_metadata.kv_indices
        if hasattr(self.rotary_emb, 'cos_sin_cache'):
            cos_sin_cache = self.rotary_emb.cos_sin_cache
        else:
            cos_sin_cache = torch.cat([self.rotary_emb.cos_cache, self.rotary_emb.sin_cache], dim=-1)
        _attn_backend = forward_batch.attn_backend
        num_kv_split = getattr(_attn_backend, 'num_kv_splits', getattr(_attn_backend, 'max_kv_splits', 16))
